...
function QuestionnaireForm() {
    ...
    const fetchTemplate = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          setFormData(prev => ({ 
            ...prev,
            intervieweeId: '', intervieweeFirstName: '', intervieweeLastName: '', intervieweePhone: '', status: '',
            answers: initialAnswers 
          }));
        } else { setError("שגיאה: תבנית השאלון לא נמצאה."); }
      } catch (err) { console.error(err); setError("שגיאה בטעינת תבנית השאלון."); } 
  ...
  const handleSaveAsException = async () => {
    ...
    try {
      ...
      alert("השאלון נשמר לטיפול המשרד.");
      resetForm();
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="actions-bar" style={{justifyContent: 'space-between'}}>
          ...
          <div>
            <button 
              type="submit" 
              disabled={isSubmitting || isCheckingId || !!idError} 
